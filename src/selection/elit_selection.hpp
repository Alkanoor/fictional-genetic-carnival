...
template <size_t N, typename T, size_t N_threads>
const std::array<N,int>& Elit_Selection<N,T,N_threads>::apply(const std::array<N,T>& qualities, int begin_at)
{
    index_after_sorting(qualities, begin_at, selected_sorted[thread_id], selected_sorted_reversed[thread_id]);
    const std::array<N,int>& temp = default_selection->apply(qualities, number_individuals_to_keep);
